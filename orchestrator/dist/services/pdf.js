...
async function spawnPythonProcess(command, args, options) {
...
async function extractPdfViaBridge(filePath, timeoutSeconds = 30, maxPages) {
    ...
    try {
        ...
        try {
            ...
            throw new PdfExtractionError('OUTPUT_ENCODING_ERROR', 'Invalid JSON response from PDF bridge', {
...
export function createPdf(env) {
    ...
    return {
        async extractText(urlOrPath, options) {
            if (!bridgeEnabled) {
        ...
        async extractTextWithMetadata(urlOrPath, options) {
            if (!bridgeEnabled) {
                throw new PdfExtractionError('PDF_BRIDGE_DISABLED', 'PDF Bridge is disabled (PDF_BRIDGE_ENABLED=false)', { bridge_enabled: false });
            }
