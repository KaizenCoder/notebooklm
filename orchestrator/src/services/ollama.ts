...
export function createOllama(env: Env) {
  ...
  return {
    ...
    async chat(model: string, messages: Array<{ role: string; content: string }>, correlationId?: string): Promise<{ message: { content: string } }> {
    ...
    async embeddings(model: string, prompt: string, correlationId?: string): Promise<number[]> {
      return withRetry(async () => {
    ...
    async checkGpu(embedModel: string, correlationId?: string): Promise<boolean> {
      return withRetry(async () => {
        const res = await request(`${base}/api/embeddings`, {
          method: 'POST',
          headers: { 'content-type': 'application/json', ...(correlationId ? { 'x-correlation-id': correlationId } : {}) },
