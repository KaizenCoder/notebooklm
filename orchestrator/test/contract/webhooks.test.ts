...
async function run() {
  ...
  // process-additional-sources
  {
    const res = await app.inject({
      method: 'POST',
      url: '/webhook/process-additional-sources',
      headers: { Authorization: 'Bearer test' },
      payload: { sources: [] },
    });
    if (res.statusCode !== 200) {
      console.error('process-additional-sources should be 200, got', res.statusCode, res.body);
      process.exit(1);
    }
    const body = res.json();
    if (!body.success || typeof body.message !== 'string' || typeof body.webhookResponse !== 'object') {
      console.error('process-additional-sources response shape invalid', body);
      process.exit(1);
    }
  }
