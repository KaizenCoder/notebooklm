...
async function findInStream(stream: string, predicate: (m: StreamMessageReply) => boolean, deadlineMs = 5000): Promise<boolean> {
  const c = createClient({ url: REDIS_URL });
  await c.connect();
  const start = Date.now();
  while (Date.now() - start < deadlineMs) {
    const msgs = await c.xRevRange(stream, '+', '-', { COUNT: 20 });
    if (msgs.some(predicate)) { await c.quit(); return true; }
...
// 1) generate-audio START/DONE
{
  ...
  const ok = await findInStream('coordination_heartbeat', (m) => {
    const f = m.message as Record<string, string>;
    return f.event === 'TTS_JOB' && f.task_id === 'nb_test' && (f.status === 'START' || f.status === 'DONE');
  });
...
// 2) process-document START/DONE
{
  ...
  const ok = await findInStream('coordination_heartbeat', (m) => {
    const f = m.message as Record<string, string>;
    return f.event === 'PROCESS_DOCUMENT' && f.task_id === 's1' && (f.status === 'START' || f.status === 'DONE');
  });
...
// 3) process-additional-sources START/DONE (copied-text)
{
  ...
  const ok = await findInStream('coordination_heartbeat', (m) => {
    const f = m.message as Record<string, string>;
    return f.event === 'PROCESS_ADDITIONAL_SOURCES' && f.task_id === 'sid1' && (f.status === 'START' || f.status === 'DONE');
  });
