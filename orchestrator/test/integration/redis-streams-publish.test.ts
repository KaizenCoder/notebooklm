...
// util
async function recent(stream: string) {
  const c = createClient({ url: REDIS_URL });
  await c.connect();
  const msgs = await c.xRevRange(stream, '+', '-', { COUNT: 5 });
  await c.quit();
  return msgs;
}
...
{
  const before = await recent('coordination_heartbeat');
  ...
  const after = await recent('coordination_heartbeat');
  if (after.length <= before.length) { console.error('no heartbeat published for generate-audio'); process.exit(1); }
}
...
// 2) process-document START/DONE
{
  ...
  await new Promise(r=>setTimeout(r, 200));
  const after = await recent('coordination_heartbeat');
  if (after.length <= before.length) { console.error('no heartbeat published for process-document'); process.exit(1); }
}
...
// 3) process-additional-sources START/DONE (copied-text)
{
  ...
  await new Promise(r=>setTimeout(r, 200));
  const after = await recent('coordination_heartbeat');
  if (after.length <= before.length) { console.error('no heartbeat published for additional-sources'); process.exit(1); }
}
