...
async function getLastId(stream: string): Promise<string> {
...
async function xreadSince(stream: string, fromId: string, predicate: (m: StreamMessageReply) => boolean, blockMs = 8000): Promise<boolean> {
  const c = createClient({ url: REDIS_URL });
  await c.connect();
  const start = Date.now();
  let lastId = fromId;
  while (Date.now() - start < blockMs) {
    const res = await c.xRead([{ key: stream, id: lastId }], { BLOCK: 1000, COUNT: 20 });
    if (res && res.length) {
...
{
  const fromId = await getLastId('coordination_heartbeat');
  const listen = xreadSince('coordination_heartbeat', fromId, (m) => {
